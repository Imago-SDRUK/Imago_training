...
Population estimates are taken from the 2021 Census, dataset TS001 – *Number of usual residents in households and communal establishments*, at MSOA level. This dataset is published by the Office for National Statistics and accessed via a stable Nomis livelink.

::: {.panel-tabset group="language"}
### Python

```{python}
url = "http://www.nomisweb.co.uk/livelinks/19099.xlsx"

population = pd.read_excel(url, skiprows=6, engine="openpyxl")

population[["MSOA21CD", "MSOA21NM"]] = (
    population["Unnamed: 0"]
    .str.split(" : ", n=1, expand=True)
)
population = population.rename(columns={"Unnamed: 1": "population"})
...
The resulting table gives one row per MSOA with the total usual resident population count, which can be merged with other MSOA-level datasets in this notebook via the `MSOA21CD` column we made.
