# Air temperature - Climate inequality {#sec-airtemp3 .unnumbered}
...
**Training approach**: The instructor demonstrates the workflow end-to-end. We use just two indicators throughout — `summer_max_tmp` and `extreme_hot_days_mean` — but the same logic transfers to any of the sixteen indicators in the product. The final section sketches three further directions students can take on their own.

------------------------------------------------------------------------

## Installing packages

For Part 3 we work entirely with the panel CSV saved in Part 2 and the MSOA-level IMD aggregate from Part 1, so we don't need any spatial or Excel-reading libraries here — just data handling, plotting, and the regression engines.
...
We also redefine a couple of small helpers that Part 2 used, so this file runs on its own:
